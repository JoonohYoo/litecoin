Marvelcoin Core version 0.13.2 is now available from:

  <https://download.marvelcoin.org/marvelcoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/marvelcoin-project/marvelcoin/issues>
...
updates will be released anymore. Without security updates, using a marvelcoin
...
ECDSA signatures inside Marvelcoin transactions now use validation using
...
Previous versions of Marvelcoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Marvelcoin Core was to raise the default minimum
relay fee.

Marvelcoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Marvelcoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running marvelcoind without having to do any manual configuration.
...
Marvelcoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Marvelcoin Core will relay transactions with
...
entirely in Marvelcoin Core 0.13, and it is at this time undecided whether the
...
Marvelcoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Marvelcoin Core automatically creates a hidden service to listen on, without
manual configuration. Marvelcoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Marvelcoin Core is listening, and
...
Marvelcoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Marvelcoin
Core to use floating fees. By default, Marvelcoin Core will use floating
fees.
...
(default: `0.0002` MVC/kB).

At all times, Marvelcoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) MVC.
Furthermore, Marvelcoin Core will never create transactions paying less than
...
`marvelcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 29423 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 29423 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /marvelcoinrpc>
        ProxyPass http://127.0.0.1:9423/
        ProxyPassReverse http://127.0.0.1:9423/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass marvelcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< marvelcoinrpc:<password>
    </Location>
...
Restarting marvelcoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in marvelcoin.conf
...
marvelcoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/marvelcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Marvelcoin Core code base has
...
- `marvelcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `marvelcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Marvelcoin scripting system that in combination with
...
Marvelcoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Marvelcoin Core.
...
The two phases are distinct in the Marvelcoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Marvelcoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Marvelcoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Marvelcoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of marvelcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of marvelcoins being spent is exactly
...
- **Increased security for multisig:** Marvelcoin addresses (both P2PKH addresses
  ...
  believed to be provided by Marvelcoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Marvelcoin's current ledger (called the UTXO set),
  ...
  note that Marvelcoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Marvelcoin users to individually opt-in to almost any change in the Marvelcoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Marvelcoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Marvelcoin
client to Marvelcoin Core 0.13.2 to signal for segwit support. When a
...
Since Marvelcoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `marvelcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `marvelcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
